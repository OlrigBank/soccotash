...
const policy: OccupancyPolicy = {
  ...
  rules:[
    ['guests',8,'bespoke'],['adults',8,'bespoke'],['children',7,'bespoke'],['infants',2,'host_decision_required'],
...
test('occupancy evaluator returns stable standard, bespoke and host-decision outcomes',()=>{
  assert.deepEqual(assessOccupancy(policy,{adults:6,children:2,infants:1,pets:0,serviceAnimals:0}),{outcome:'standard',reasons:[]});
  const combined=assessOccupancy(policy,{adults:8,children:3,infants:0,pets:0,serviceAnimals:0});
  assert.equal(combined.outcome,'bespoke');assert.equal(combined.reasons[0].code,'guests_standard_threshold_exceeded');
  const bespoke=assessOccupancy(policy,{adults:9,children:0,infants:0,pets:0,serviceAnimals:0});
  assert.equal(bespoke.outcome,'bespoke');assert.ok(bespoke.reasons.some(reason=>reason.code==='adults_standard_threshold_exceeded'));
...
test('occupancy administration remains authenticated, same-origin and closed-rule based',async()=>{
  const [migration,combinedMigration,api,page]=await Promise.all([
    readFile(new URL('../../db/047_occupancy_policy_lifecycle.sql',import.meta.url),'utf8'),
    readFile(new URL('../../db/052_combined_guest_occupancy.sql',import.meta.url),'utf8'),
  ...
  assert.match(combinedMigration,/subject IN \('guests', 'adults', 'children', 'infants', 'pets', 'service_animals'\)/);
  assert.match(combinedMigration,/WHERE subject = 'adults'/);
...
test('public booking captures categories and snapshots policy assessment without a capacity clamp',async()=>{
  const [form,quote,booking,migration]=await Promise.all([
    readFile(new URL('../../src/components/CompactBookingPanel.astro',import.meta.url),'utf8'),
  ...
  assert.match(form,/Ages 13\+/);assert.match(form,/Ages 2–12/);assert.match(form,/Under 2/);
