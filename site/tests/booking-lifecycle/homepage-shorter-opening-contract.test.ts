...
test('the semantic comparison retains three standard choices, Bespoke and a route to Jenna', async () => {
  ...
  assert.match(homepage, /<table>[\s\S]*<caption>Swipe sideways to compare all stays\.<\/caption>/);
  assert.match(homepage, /<th scope="col"><a href="\/listings\/olrig-bank\/">Olrig Bank<\/a><\/th>/);
  assert.match(homepage, /<th scope="col"><a href="\/listings\/event\/">Olrig Bank\+\+<\/a><\/th>/);
  assert.match(homepage, /<th scope="col"><a href="\/listings\/cottage\/">Cottage at Olrig Bank<\/a><\/th>/);
  assert.match(homepage, /<th scope="row">Maximum guests<\/th>\s*<td>8<\/td>\s*<td>12<\/td>\s*<td>4<\/td>/);
  assert.match(homepage, /<th scope="row">Bedrooms<\/th>\s*<td>4<\/td>\s*<td>6<\/td>\s*<td>2<\/td>/);
  assert.match(homepage, /<th scope="row">Bathrooms<\/th>\s*<td>2<\/td>\s*<td>3<\/td>\s*<td>1<\/td>/);
...
test('the comparison scrolls within its region at narrow widths', async () => {
  ...
  assert.match(homepage, /\.ways-to-stay table\s*\{[\s\S]*min-width:\s*44rem/);
  assert.match(homepage, /:is\(thead th:first-child, tbody th\)[\s\S]*position:\s*sticky[\s\S]*left:\s*0/);
  assert.equal(homepage.match(/Swipe sideways to compare all stays\./g)?.length, 1);
});
