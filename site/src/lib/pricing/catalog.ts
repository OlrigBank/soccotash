...
export const RULE_CATALOG: RuleCatalogEntry[] = [
  ...
  { type: 'price_floor', label: 'Price floor', description: 'Prevent accommodation from falling below a nightly or stay-total minimum.', category: 'Base pricing' },
  ...
  { type: 'maximum_stay', label: 'Maximum stay', description: 'Reject stays longer than the configured maximum.', category: 'Stay rules' },
  { type: 'arrival_day_restriction', label: 'Allowed arrival days', description: 'Permit check-in only on selected weekdays.', category: 'Stay rules' },
  { type: 'departure_day_restriction', label: 'Allowed departure days', description: 'Permit check-out only on selected weekdays.', category: 'Stay rules' },
  { type: 'fixed_package', label: 'Fixed stay package', description: 'Use a fixed total for an exact number of nights and optionally include fees.', category: 'Stay rules' },
...
export function defaultRuleValues(type: PricingRuleType): {
  ...
  switch (type) {
    ...
    case 'price_floor':
      return { name, conditions: {}, action: { amountPence: 30000, floorBasis: 'nightly' }, priority: 10, stackable: false, stackingGroup: 'price-floor' };
    ...
    case 'maximum_stay':
      return { name, conditions: {}, action: { nights: 28 }, priority: 100, stackable: true, stackingGroup: 'stay-restriction' };
    case 'arrival_day_restriction':
      return { name, conditions: {}, action: { daysOfWeek: [1, 5] }, priority: 100, stackable: true, stackingGroup: 'arrival-days' };
    case 'departure_day_restriction':
      return { name, conditions: {}, action: { daysOfWeek: [1, 5] }, priority: 100, stackable: true, stackingGroup: 'departure-days' };
    case 'fixed_package':
      return { name, conditions: {}, action: { nights: 3, amountPence: 163500, includesCleaning: true, includesPetFee: false }, priority: 80, stackable: false, stackingGroup: 'stay-package' };
