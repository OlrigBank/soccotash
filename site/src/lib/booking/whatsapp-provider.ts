...
export async function sendWhatsAppText(input: {
  ...
  const configuration = getWhatsAppConfiguration();
  const inboundReplyEnabled = String(process.env.WHATSAPP_INBOUND_AUTO_REPLY_ENABLED || '').trim().toLowerCase() === 'true';
  if (!inboundReplyEnabled || !configuration.configured || !configuration.provider) {
    throw new Error('WHATSAPP_INBOUND_REPLY_NOT_CONFIGURED');
  }
  const response = await fetch(`https://graph.facebook.com/${configuration.graphVersion}/${configuration.phoneNumberId}/messages`, {
    method: 'POST',
